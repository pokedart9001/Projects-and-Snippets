...
void find_free(size_t size, node_t **found, node_t **previous) {
  // TODO
	for (node_t *curr = heap(), *prev = NULL; curr != NULL; curr = curr->next, prev = curr) {
...
void split(size_t size, node_t **previous, node_t **free_block, header_t **allocated) {
  ...
  // TODO
	node_t *old_free_block = *free_block;
	*free_block = (node_t *)((char *)*free_block + size + sizeof(header_t));
	(*free_block)->size = old_free_block->size - size - sizeof(header_t);
	if (*previous == NULL) {
		head = *free_block;
	} else {
		*previous = *free_block;
	}
	*allocated = (header_t *)old_free_block;
	(*allocated)->size = size; (*allocated)->magic = MAGIC;
}
...
void *my_malloc(size_t size) {
  // TODO
	node_t *found = NULL, *previous = NULL;
...
// Checks whether the next free node is adjacent to the current node.
// This function is a helper function for `coalesce`.
//
// PARAMETERS:
...
//
bool isAdjacentFree(node_t *node) {
	return (node_t *)((char *)node + node->size + sizeof(node_t)) == node->next;
}
...
void coalesce(node_t *free_block) {
  // TODO
	for (node_t *curr = free_block; curr->next != NULL;) {
		if (isAdjacentFree(curr)) {
...
void my_free(void *allocated) {
  // TODO
	header_t *allocated_block = (header_t *)((char *)allocated - sizeof(header_t));
	assert(allocated_block->magic == MAGIC);
	node_t *free_block = (node_t *)allocated_block;
	free_block->size = allocated_block->size;
	free_block->next = heap();
	head = free_block;
	coalesce(heap());
}
