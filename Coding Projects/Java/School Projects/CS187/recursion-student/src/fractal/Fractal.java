...
public class Fractal {
	...
	private void drawKochCurve(Point2D p1, Point2D p2, int level) {
		...
		double thirdDist = p1.distance(p2)/3;
		Point2D[] points = new Point2D[5];
		Turtle turtle = new Turtle(p1, p2);
		
		points[0] = p1;
		turtle.move(thirdDist);
		points[1] = turtle.getPosition();
		turtle.turnLeft(60);
		turtle.move(thirdDist);
		points[2] = turtle.getPosition();
		turtle.turnRight(120);
		turtle.move(thirdDist);
		points[3] = turtle.getPosition();
		points[4] = p2;
		
		for (int i = 0; i < points.length - 1; i++)
			drawKochCurve(points[i], points[i+1], level - 1);
	}
	...
	private void drawSierpinskiTriangle(Point2D p1, Point2D p2, Point2D p3, int level) {
		...
		double halfDist = p1.distance(p2)/2;
		Turtle turtle = new Turtle(p1, p2);
		...
		for (int i = 0; i < 3; i++)
		{
			...
			for (int j = 0; j < 3; j++)
			{
				points[j] = turtle.getPosition();
				turtle.move(halfDist);
				turtle.turnLeft(120);
			}
			drawSierpinskiTriangle(points[0], points[1], points[2], level - 1);
			turtle.move(halfDist*2);
			turtle.turnLeft(120);
		}
	}
	...
	private void drawSierpinskiCarpet(Point2D p, double a, int level) {
		...
		Turtle turtle = new Turtle(p, new Point2D.Double(p.getX(), p.getY()+1));
		for (int i = 0; i < 4; i++)
		{
			for (int j = 0; j < 2; j++)
			{
				drawSierpinskiCarpet(turtle.getPosition(), a/3, level - 1);
				turtle.move(a/3);
			}
			turtle.turnRight(90);
		}
	...
	void drawMyFractal(Point2D p1, Point2D p2, int level) {
		...
		points[3] = turtle.getPosition();
		turtle.move(dist);
		points[4] = turtle.getPosition();
		turtle.turnLeft(90);
		turtle.move(dist);
		points[5] = turtle.getPosition();
		points[6] = p2;
		
		for (int i = 0; i < points.length - 1; i++)
			drawMyFractal(points[i], points[i+1], level - 1);
	}
