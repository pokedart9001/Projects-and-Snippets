...
public abstract class Searcher<T> {
	...
	public final boolean isValidSolution(List<T> solution) {
		...
		for (int i = 0; i < solution.size() - 1; i++) {
			if (!searchProblem.getSuccessors(solution.get(i))
					.contains(solution.get(i + 1)))
				return false;
		}
		return solution.get(0).equals(searchProblem.getInitialState())
				&& searchProblem.isGoal(solution.get(solution.size() - 1));
	}
}
